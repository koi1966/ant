...
create procedure DBA.pr_dogovor_delete(@dogovor char(7))
/* RESULT ( column_name, ... ) */
begin
  declare @nza numeric;
  declare @time_x date;
...
go
COMMENT TO PRESERVE FORMAT ON PROCEDURE "DBA"."pr_dogovor_delete" IS 
{create PROCEDURE DBA."pr_dogovor_delete" (@dogovor char(7) )
/* RESULT ( column_name, ... ) */
BEGIN
declare  @nza        numeric;
...
create procedure DBA.pr_dogovor_vork(@dogovor char(7))
/* RESULT ( column_name, ... ) */
begin
  declare @nza numeric;
  declare @time_x date;
  ...
  begin
    ...
    // удаляем
    delete from dogovor_stop_arhiv where dogovor = @dogovor;
    //
    // копируем данные в архив, а зате удаляем
    //insert into dba.karta_arhiv select * from karta where karta.dogovor = @dogovor;
    // удаляем
    delete from karta_arhiv where dogovor = @dogovor;
    //
    // копируем данные в архив, а зате удаляем
    //insert into dba.dogovor_arhiv select * from dogovor where dogovor.dogovor = @dogovor;
    // Удаляем
    delete from dogovor_arhiv where dogovor = @dogovor;
    //
    //             delete from tarif where dogovor = @dogovor;
    //insert into dba.tarif_arhiv select * from tarif where tarif.dogovor = @dogovor;
    // Удаляем
    delete from tarif_arhiv where dogovor = @dogovor
  //
  end
//end if;	;
end
go
COMMENT TO PRESERVE FORMAT ON PROCEDURE "DBA"."pr_dogovor_vork" IS 
{create PROCEDURE DBA."pr_dogovor_vork" (@dogovor char(7) )
/* RESULT ( column_name, ... ) */
BEGIN
declare  @nza        numeric;
...
begin
  ...
  if new_name.Annulir = '0' then
    insert into dba.tarif(tarif,data_n,Data_K,status,dogovor,symma) values(
      new_name.tarif,new_name.data_tar,new_name.data_tar,'0',new_name.dogovor,0)
  end if;
  if new_name.Annulir = '4' then
    insert into dba.tarif(tarif,data_n,Data_K,status,dogovor,symma) values(
      new_name.tarif,new_name.data_tar,new_name.data_tar,'4',new_name.dogovor,0)
  end if
end
go
...
create trigger tr_data_pley_update after update of data_pley
order 1 on DBA.dogovor
referencing old as old_name new as new_name
for each statement
...
begin
  if new_name.Annulir = '4' then
    insert into dba.tarif(tarif,data_n,data_k,status,dogovor,symma) values(
      new_name.tarif,new_name.data_pley,new_name.data_pley,new_name.Annulir,new_name.dogovor,0)
  end if
end
go
COMMENT TO PRESERVE FORMAT ON TRIGGER "DBA"."dogovor"."tr_data_pley_update" IS 
{create TRIGGER tr_data_pley_update AFTER UPDATE OF "data_pley"
	ORDER 1 ON "DBA"."dogovor"
	 REFERENCING OLD AS old_name NEW AS new_name
	FOR EACH STATEMENT
...
for each row
begin
  // Вставляем данные в таблицу  -   tarif
  // Если  0 - простой договор
  // Если  1 - приостановленный договор
  // Если  2 - аннулированный договор
  // Если  4 - кабельный договор
  // Если  5 - кабельный договор без ввода, его в таблицу тариф не записывать.
  if new_name.Annulir = '0' then
    insert into dba.tarif(tarif,data_n,data_k,status,dogovor,symma) values(
      new_name.tarif,new_name.data,new_name.data,new_name.Annulir,new_name.dogovor,0)
  end if;
  if new_name.Annulir = '4' then
    insert into dba.tarif(tarif,data_n,data_k,status,dogovor,symma) values(
      new_name.tarif,new_name.data_pley,new_name.data_pley,new_name.Annulir,new_name.dogovor,0)
  end if;
  // Вставляем данные в таблицу  -   karta
...
IF new_name.Annulir = '0' THEN
   insert into dba.tarif (tarif, data_n,data_k, status, dogovor,symma) 
   values (new_name.tarif, new_name.data,new_name.data, new_name.Annulir, new_name.dogovor,0)
END IF;
...
create trigger tr_plus_plata before delete order 1 on
DBA.karta
...
begin
  update dba.dogovor set
    ostatok = ostatok+old_.Summ where
    dogovor = old_.dogovor
end
go
COMMENT TO PRESERVE FORMAT ON TRIGGER "DBA"."karta"."tr_plus_plata" IS 
{create TRIGGER tr_plus_plata BEFORE DELETE
...
create trigger tr_plata before insert order 1 on
DBA.karta
referencing new as new_name
for each row
/* WHEN ( search_condition ) */
begin
  update dba.dogovor set
    ostatok = ostatok-new_name.Summ where
    dogovor = new_name.dogovor
end
go
COMMENT TO PRESERVE FORMAT ON TRIGGER "DBA"."karta"."tr_plata" IS 
{create TRIGGER tr_plata BEFORE INSERT
	ORDER 1 ON "DBA"."karta"
	 REFERENCING NEW AS new_name 
	FOR EACH ROW
	/* WHEN ( search_condition ) */
...
create trigger rt_update_strit before update of Strit
...
begin
  ...
  if "left"(old_name.jek,2) <> 'КТ' then
    update dba.karta set strit = new_name.strit where
      /* условие упдейта */
      /* дочернее = ведомое */
      strit = old_name.strit and
      "left"(dogovor,2) = old_name.jek;
    /* IF left(old_name.jek,2) != 'КТ' THEN */
    update dba.dogovor set strit = new_name.strit where
      strit = old_name.strit and
      "left"(dogovor,2) = old_name.jek
  end if;
...
go
COMMENT TO PRESERVE FORMAT ON TRIGGER "DBA"."strit"."rt_update_strit" IS 
{create TRIGGER rt_update_strit BEFORE UPDATE OF "Strit"
	ORDER 1 ON "DBA"."strit"
	REFERENCING OLD AS old_name NEW AS new_name 
	FOR EACH ROW
...
create trigger tr_nachislenie_k before update of data_nachisl_k
order 2 on DBA.dat
referencing old as old_name new as new_name
for each row
/* WHEN ( search_condition ) */
begin
  update dba.dogovor set
    ...
    dogovor not like '20%' and
    // учитывать дату заключения договора
    data < new_name.data_nachisl_k;
  update dba.tarif set
    data_K = new_name.data_nachisl_k where
    Status = '4'
// WHERE Status in ('0','4')	;
end
go
COMMENT TO PRESERVE FORMAT ON TRIGGER "DBA"."dat"."tr_nachislenie_k" IS 
{create TRIGGER tr_nachislenie_k BEFORE UPDATE OF "data_nachisl_k"
	ORDER 2 ON "DBA"."dat"
	 REFERENCING OLD AS old_name NEW AS new_name 
	FOR EACH ROW
...
create trigger tr_delete_oborotna before delete order 1 on
DBA.ob_vedomost
referencing old as old_
for each row
/* WHEN ( search_condition ) */
...
go
COMMENT TO PRESERVE FORMAT ON TRIGGER "DBA"."ob_vedomost"."tr_delete_oborotna" IS 
{create TRIGGER tr_delete_oborotna BEFORE DELETE
...
create trigger tr_delete_oborotna_kab before delete order 1 on
DBA.ob_vedomost_kab
referencing old as old_
for each row
/* WHEN ( search_condition ) */
...
go
COMMENT TO PRESERVE FORMAT ON TRIGGER "DBA"."ob_vedomost_kab"."tr_delete_oborotna_kab" IS 
{create TRIGGER tr_delete_oborotna_kab BEFORE DELETE
